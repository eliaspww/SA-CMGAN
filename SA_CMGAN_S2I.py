...
class S2I(object):
    ...
    def discriminator(self, x_sound, x_img, is_training=True, reuse=False):
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            net = self.attention_Img(net, scope="attention", reuse=reuse, sn=True)
    ...
    def attention_Img(self, x, is_training=True, scope='attention_Img', reuse=False, sn=False):
        with tf.variable_scope(scope, reuse=reuse):
            f = tf.nn.relu(bn(conv2d(x, 8, 4, 4, 1, 1, name='fI_conv', sn=sn), is_training=is_training, scope='fI_bn'))
            g = tf.nn.relu(bn(conv2d(x, 8, 4, 4, 1, 1, name='gI_conv', sn=sn), is_training=is_training, scope='gI_bn'))
            h = tf.nn.relu(bn(conv2d(x, 64, 4, 4, 1, 1, name='hI_conv', sn=sn), is_training=is_training, scope='hI_bn'))
    ...
    def attention_Sud(self, x, is_training=True, scope='attention_Sud', reuse=False):
        with tf.variable_scope(scope, reuse=reuse):
            f = tf.nn.relu(bn(conv2d(x, 8, 4, 4, 1, 1, name='fS_conv'), is_training=is_training, scope='fS_bn'))
            g = tf.nn.relu(bn(conv2d(x, 8, 4, 4, 1, 1, name='gS_conv'), is_training=is_training, scope='gS_bn'))
            h = tf.nn.relu(bn(conv2d(x, 64, 4, 4, 1, 1, name='hS_conv'), is_training=is_training, scope='hS_bn'))
    ...
    def build_model(self):
        ...
        self.inputs_img_mis = tf.placeholder(tf.float32, [bs] + image_dims, name='mis_images')
        ...
        G_img = self.generator(self.z, En_Sud, is_training=True, reuse=False)

        D_real, D_real_logits = self.discriminator(En_Sud, self.inputs_img, is_training=True, reuse=False)

        D_fake, D_fake_logits = self.discriminator(En_Sud, G_img, is_training=True, reuse=True)

        D_mis, D_mis_logits = self.discriminator(En_Sud, self.inputs_img_mis, is_training=True, reuse=True)
        ...
        self.fake_imgs = self.generator(self.z, self.sound, is_training=False, reuse=True)
    ...
    def train(self):
        ...
        if not could_load:
            ...
            self.save_pretrain(self.checkpoint_dir, counter)
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.num_batches):
                ...
                train_feed_dict_d = {
                    ...
                    self.inputs_img: batch_images,
                    self.inputs_sound: batch_sounds,
                    self.inputs_img_mis: batch_mis_imgs
                }
                train_feed_dict_g = {
                    self.z: batch_z,
                    self.inputs_sound: batch_sounds
                ...
                if np.mod(idx+1, 1000) == 0:
                    start_batch_id = 0
                    samples = self.sess.run(self.fake_imgs,
                                            feed_dict=train_feed_dict_g)
    ...
    def visualize_results(self, epoch, idx):
        ...
        samples = self.sess.run(self.fake_imgs, feed_dict={self.z: z_sample, self.inputs_sound: batch_sounds})

        prediction_Img = self.sess.run(self.prediction_Img, feed_dict={self.inputs_img:samples})
        ...
        result_Img = self.sess.run(accuracy_Img, feed_dict={self.inputs_img:samples, self.y:batch_labels})
        print(epoch, " accuracy_Img:",  result_Img)

        prediction_Sud = self.sess.run(self.prediction_Sud, feed_dict={self.inputs_sound: batch_sounds})
        ...
        result_Sud = self.sess.run(accuracy_Sud, feed_dict={self.inputs_sound:batch_sounds, self.y:batch_labels})
    ...
    def visualize_results_test(self, epoch):
        ...
        result_Sud = self.sess.run(accuracy_Sud,
                                   feed_dict={self.inputs_sound: batch_sounds, self.y: batch_labels})
    ...
    def visualize_paper(self, epoch):
        ...
        samples = self.sess.run(self.fake_imgs, feed_dict={self.z: z_sample, self.inputs_sound: batch_sounds})
        si = [4, 14, 21, 14, 23, 31, 34, 41, 43, 53, 58, 61, 63]
        ...
        save_images(canvas, [3, len_discrete_code_ply],
                    check_folder(self.test_dir + '/' + self.model_dir) + '/' + self.model_name + '_no_sn.png')
    ...
    def save_pretrain(self, checkpoint_dir, step):
        checkpoint_dir = os.path.join(checkpoint_dir, "pretrain", self.model_name)
